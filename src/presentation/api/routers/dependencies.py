...
def get_dependencies(task_id: int, db: DbDep) -> TaskDependenciesResponse:
    uc = DependencyUseCases(db)
    data = uc.get_dependencies(task_id)
...
def add_dependency(task_id: int, body: DependencyCreateRequest, db: DbDep) -> DependencyResponse:
    uc = DependencyUseCases(db)
...
def remove_dependency(task_id: int, predecessor_id: int, db: DbDep) -> None:
    uc = DependencyUseCases(db)
    uc.remove_dependency(successor_task_id=task_id, predecessor_task_id=predecessor_id)
