...
高速化したい場合は `--skip-tests` や `--skip-frontend-install` を指定できます。frontend build は lockfile がある場合 `npm ci` を試し、package.json と lockfile がずれている場合は `npm install` にフォールバックします。backend build は `uv` があれば `uv run`、なければ実行中の Python に必要な dev dependencies を `pip install -e . ruff pytest httpx` で補完してから `python -m ruff` / `python -m pytest` にフォールバックします。
